...
def draw_boxes(frame, detections: list[dict], class_names=None, color=(0, 255, 0), thickness=2):
    """
    Vẽ bounding boxes và putText lên ảnh.

    Args:
        frame (numpy.ndarray): The input frame.
        detections (list[dict]): danh sách các detections, mỗi detection là một dict chứa 'bbox', 'conf', và 'cls'.
    ...
    for detection in detections:
        ...
        if 'speed' in detection:
            text = f"{label} - ID: {detection['track_id']}: {conf:.2f}: {detection['speed']:.2f} km/h"
        else:
        ...
        cv2.putText(frame, text, (x1 + 2, y1 - 2), cv2.FONT_HERSHEY_SIMPLEX, 0.5, [0, 0, 0], 2)
    return frame